...
module declarative where
  ...
  dex2 : ∅ ⊢ (N 3) ∘ (N 4) :: <||>
  dex2 = DAp (DSubsume (DSubsume DNum TCHole2) TCHole1) DNum
  ...
  bidirdec (Inl (SAp x x₁ x₂)) (ERAAp er er₁) = DAp {!!} {!!}
  bidirdec (Inl SNum) ERANum = DNum
  bidirdec (Inl (SPlus x x₁)) (ERAPlus er er₁) = DPlus {!!} {!!}
  ...
  bidirdec (Inr (ALam x₁ x₂ x₃)) (ERALam er) = {!DLam!}
